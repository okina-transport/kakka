...
public class TiamatChangeLogExportRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:processTiamatChangeLogExportTask")
                ...
                .process(e -> e.getIn().setHeader(Constants.QUERY_STRING, createQueryUrl(e)))
                .to("direct:exportChangedStopPlaces")

                .choice().when(body().isNull())
                .log(LoggingLevel.INFO, "Finished Tiamat publish export: ${exchangeProperty." + Constants.TIAMAT_EXPORT_TASKS + ".currentTask.name} with no new data")
                .otherwise()
                ...
                .log(LoggingLevel.INFO, "Finished Tiamat publish export: ${exchangeProperty." + Constants.TIAMAT_EXPORT_TASKS + ".currentTask.name}. Uploaded new file ${header." + Constants.FILE_HANDLE + "}")
        ...
        from("direct:fetchEtcdStateForExport")
                .setHeader(Constants.ETCD_KEY, simple(etcdKeyPrefix + "/${exchangeProperty." + Constants.TIAMAT_EXPORT_TASKS + ".currentTask.name}_cnt"))
                .to("direct:getEtcdValue")
                .process(e -> e.getIn().setHeader(CNT, e.getIn().getBody() == null ? 0 : e.getIn().getBody(Integer.class)))

                .setHeader(Constants.ETCD_KEY, simple(etcdKeyPrefix + "/${exchangeProperty." + Constants.TIAMAT_EXPORT_TASKS + ".currentTask.name}_to"))
                .to("direct:getEtcdValue")
                .choice().when(body().isNull())
    ...
    private String createQueryUrl(Exchange e) {
        TiamatExportTask task = e.getProperty(Constants.TIAMAT_EXPORT_TASKS, TiamatExportTasks.class).getCurrentTask();
